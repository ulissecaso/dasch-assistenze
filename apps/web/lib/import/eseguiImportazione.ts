...
export async function eseguiImportazioneCsv(
  ...
  await eseguiConConcorrenza(praticheEsistentiDaConfrontare, CONCORRENZA_PRATICHE, async (pratica: any) => {
    try {
      ...
      // "consegna" pero' anche data_consegna_prevista puo' cambiare nel
      // tempo su Vamart (l'operatore sposta la data), e va tenuta
      // allineata, altrimenti resta congelata al valore letto la prima
      // volta che la pratica e' stata creata (bug segnalato: la dashboard
      // Monitor Consegne non si accorge mai degli spostamenti di data).
      const aggiornamentiPratica: Record<string, any> = {};
...
async function sincronizzaFasiConsegna(
  ...
  // pratica_fasi.data_prevista resta congelata alla stima fatta dal
  // trigger DB alla creazione della pratica (now() + SLA ore, vedi
  ...
  // pratica di consegna non "sparisce"/non cambia mai posizione nel
  // Monitor Consegne anche dopo aver spostato la data di consegna.
