...
// FIX 28/07/2026 (segnalato da Lucia, operatrice: clienti dello store di
// Roma come "Mariani Lorenzo" restavano visibili tra le segnalazioni degli
// operatori). Causa: il trigger su commissioni_escluse (migrazione 0019)
// annulla solo le pratiche il cui codice_commissione coincide ESATTAMENTE
// con quello Vamart. Ma una pratica nata da segnalazione mail/app (caso 1
// nel commento in cima al file) ha come codice_commissione il riferimento
// citato dal cliente nella mail, quasi sempre diverso dal numero che Vamart
// assegnera' poi alla commissione vera: il trigger quindi non la trova mai,
// e restava "in attesa" per sempre, visibile e assegnata a un operatore.
// Questa funzione cerca (stessa tecnica di match nome+finestra di date gia'
// usata piu' sotto per i casi normali) le pratiche ancora aperte per lo
// stesso cliente e le annulla esplicitamente, con una nota che spiega il
// motivo (recuperabili comunque da admin -> Gestione pratiche -> "Riattiva"
// in caso di falso positivo).
async function annullaPraticheInAttesaEscluse(supabase, brandId, riga, codiceCommissione) {
const nomeCompleto = [riga.cognome, riga.nome].filter(Boolean).join(" ").trim();
...
for (const pratica of candidati) {
const nota = `Annullata automaticamente: la commissione ${codiceCommissione} appartiene allo store di Roma (venditore ${riga.venditore}), che segue l'assistenza dei propri clienti per conto proprio. Non deve comparire tra le pratiche degli operatori Dasch.`;
await supabase
.from("pratiche")
.update({
stato_generale: "annullata",
data_chiusura_effettiva: new Date().toISOString(),
descrizione: pratica.descrizione ? `${pratica.descrizione}\n\n${nota}` : nota,
})
.eq("id", pratica.id);
...
valore_nuovo: "annullata (esclusa automaticamente: cliente store di Roma)",
origine: "automazione",
});
}
...
// codice_commissione E' davvero un id Vamart comparabile). Le pratiche
...
let nuove = 0, ricollegate = 0, giaPresenti = 0, riclassificate = 0, errori = 0, escluse = 0, esclusePraticheAnnullate = 0;
...
//
// FIX 28/07/2026: il trigger sopra copre solo le pratiche il cui
// codice_commissione coincide con quello Vamart. Le pratiche "in
// attesa" nate da segnalazione mail/app (caso 1 in cima al file) hanno
// invece come codice_commissione il riferimento citato dal cliente, e
// restavano quindi visibili/assegnate per sempre agli operatori (mai
// annullate). annullaPraticheInAttesaEscluse cerca e chiude queste
// pratiche per nome cliente + finestra di date, la stessa tecnica di
// match gia' usata piu' sotto per il caso normale.
...
const annullate = await annullaPraticheInAttesaEscluse(supabase, brandId, riga, codiceCommissione);
esclusePraticheAnnullate += annullate;
...
`escluse (store di Roma): ${escluse}, pratiche in attesa annullate per store di Roma: ${esclusePraticheAnnullate}, ` +
`errori: ${errori}, ` +
